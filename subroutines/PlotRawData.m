function PlotRawData(traces, myfile)
...
nexttile(1,[2 2]);
        scatter3(traces(:,1), traces(:,2), traces(:,3),5,traces(:,5), 'filled');  % color options: ClusterIDs, myXYZ(:,5)
        colormap parula
        axis equal
        title(strcat(myfile,' raw data'));
        ax=gca;
       ax.TickDir="out";
nexttile
...
nexttile
    ...
    StDevHist = 1e9*StDevALL(StDevALL(:,1)>0,:);  
        ax=gca;
       ax.TickDir="out";
    
nexttile
       histogram(StDevHist(:,1),'BinWidth', 1);
       xlim([0 30])
       title("STD-X");
        ax=gca;
       ax.TickDir="out";
nexttile
        histogram(StDevHist(:,2),'BinWidth', 1);
        xlim([0 30])
        title("STD-Y");
         ax=gca;
       ax.TickDir="out";
nexttile
        histogram(StDevHist(:,3),'BinWidth', 1);
        xlim([0 30])
        title("STD-Z");
        ax=gca;
       ax.TickDir="out";
