%% Script for the analysis of MINFLUX signals from DNA-Paint labelled ALFA-tagged PIEZO1
    ...
    result.AllStd = [];
    result.Dimers = [];
    result.DimerDistances = [];
    result.Monomers = [];

%% %%%%%%%%%%%%%%%%%%%% – Load data – %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

   % choose which data to analyse: 0 = P1-CTL, 1 = P1-WedgeDel
   
    DataSource = 1;

    if DataSource == 0
        load("PIEZO1_ALFAmGL_CTL_RawData.mat");
         myStruct = PIEZO1_ALFAmGL_CTL_RawData;
   Exclude = [21 36 38 40 50 65];     %%trimers excluded because the signals are not likely in the plasma membrane
         result.color = [0 0.5 0];

    elseif DataSource == 1
        load("PIEZO1_ALFAmGL_WedgeDel_RawData.mat");
         myStruct = PIEZO1_ALFAmGL_WedgeDel_RawData;
        result.color = [0.6 0.4 0.8];
    Exclude = [3 5 10 15 16 23 25 33 35]; %% trimers excluded because the signals are not likely in the plasma membrane
...
for k = 1:length(myStruct)      % loop through all files
  % for k = 1                  % loop through indicated files
 ...
 %%  %%%%%%%%%%%%%%%%%%%%%%%% – set analysis parameters – %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [p]=SetALFAanalysisParameters_V2(); % set analysis paramters
...
%%  %%%%%%%%%%%%%%%%%%%%%%%% – run DBSCAN to find signals from the same fluorophore – %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    result.AllStd = cat(1,result.AllStd, STDforeachiteration);
...
% Find individual fluorophore with no neighbour within 60nm
% Initialize monomers array
Monomers = [];

% First pass: find all points with no neighbors within 60nm
for i = 1:size(traces_AVG, 1)
    if H(2,i) > p.MinTrimerDist
        Monomers = [Monomers; i];
    end
end

% Find dimers: 2 fluorophores within 40nm, with no neighbour within 60nm
% Initialize dimers array
Dimers = zeros(0, 2);  % This ensures it always has 2 columns even when empty
DimerDistances = [];

% Find all possible dimers
for i = 1:size(traces_AVG, 1)
    % Check if second nearest neighbor is within 40nm (dimer condition)
    if H(2,i) <= 40 && H(3,i) > p.MinTrimerDist
        % Get the pair (sorted to avoid duplicates like [1,2] vs [2,1])
        pair = sort([i, Hidx(2,i)]);
        
        % Check if this pair hasn't been added already
        % First check if Dimers is not empty to avoid dimension mismatch
        if isempty(Dimers) || ~any(all(Dimers == pair, 2))
            Dimers = [Dimers; pair];
            DimerDistances = [DimerDistances; H(2,i)];
        end
    end
end
%
    ...
    for i=1:length(H)
        ...
        % Get all trimers with additional NN within 40-60
        % if(H(4,i)>40-60 && H(4,Hidx(2,i))>40-60 && H(4,Hidx(3,i))>40-60)
...
% After detecting all trimers, dimers, and monomers, ensure:
% 1. Monomers are not part of any dimer or trimer
% 2. Dimers are not part of any trimer

% First, get all points that are in trimers
trimer_points = [];
if ~isempty(Trimers)
    trimer_points = unique(Trimers(:));
end

% Then get all points that are in dimers
dimer_points = [];
if ~isempty(Dimers)
    dimer_points = unique(Dimers(:));
end

% Remove from monomers any points that are in dimers or trimers
Monomers = setdiff(Monomers, union(trimer_points, dimer_points));

% Remove from dimers any pairs where either point is in a trimer
if ~isempty(Dimers) && ~isempty(Trimers)
    to_keep = ~any(ismember(Dimers, trimer_points), 2);
    Dimers = Dimers(to_keep, :);
    DimerDistances = DimerDistances(to_keep);
end

% store the results

result.Monomers = cat(1, result.Monomers, Monomers);
result.Dimers = cat(1, result.Dimers, Dimers);
result.DimerDistances = cat(1, result.DimerDistances, DimerDistances);
...
clear myfile UpperZ LowerZ include result.TrimerCount Tidx DistMatrix Trimers data Dimers DimerDistances dimer_points Monomers to_keep pair
clear Std_tid Numtraceforeachiteration NearestNeighbor aggregateTraces TrimTrace SizeA SizeB SizeC 
