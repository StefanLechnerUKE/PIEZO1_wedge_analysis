...
PlotTrimer3D = false;
PlotContours = false;
PlotDensityAllTrimers = false;


%Selected examples to plot 
    % CTL
     selectedIndices = [22 58 88 86 85 82 80 111];
     
    % WedgeDel
    % selectedIndices = [95 92 85 29 38 28 20 11];
...
% for k=1:size(fns,1) 
  % figure('Position',[100 100 1100 300]);
    % tiledlayout(2,8);
     ...
     % hold on;
%% options to only plpt given examples   
for currentIdx  = 1:length(selectedIndices)
    k = selectedIndices(currentIdx )
%%
    % nexttile

    % extract protamer coordinates //  option to plot density from all channels   
    ...
    if PlotTrimer3D
        ...
        scatter3(TrimAtrans2(:,1),TrimAtrans2(:,2),TrimAtrans2(:,3),120,color1,'filled','MarkerFaceAlpha',0.5);
        hold on;
        scatter3(TrimBtrans2(:,1),TrimBtrans2(:,2),TrimBtrans2(:,3),120,color2,'filled','MarkerFaceAlpha',0.5);
        scatter3(TrimCtrans2(:,1),TrimCtrans2(:,2),TrimCtrans2(:,3),120,color3,'filled','MarkerFaceAlpha',0.5);
    ...
    %% Plot given examples - Density map only
    ...
    % %  ylabel('(nm)');
    % % ax = gca; ax.XAxis.FontSize = 8; ax.YAxis.FontSize = 8;
    % % c = colorbar; c.Label.String = 'probability'; c.Label.FontSize = 10; c.FontSize = 10;
    % %     ax.Box = 'on';
    % % ax.BoxStyle = 'full';
...
%     ax.BoxStyle = 'full';
% 
% % Configure figure for landscape PDF export
% fig = gcf; % Get current figure handle
% fig.PaperOrientation = 'landscape'; % Set landscape orientation
% fig.PaperUnits = 'inches'; % Set paper units
% fig.PaperSize = [11.69 8.27]; % Letter size landscape (or choose another size)
% fig.PaperPosition = [0 0 11 8.5]; % Full page
% 
% 
%     exportgraphics(fig, 'P1-Trimers-Soma-CTL-Supercurved.pdf', ...
%     'ContentType', 'vector', ... % Vector graphics for quality
